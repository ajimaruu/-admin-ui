import React, { useState } from 'react';
import { useFormik } from 'formik';
import * as Yup from 'yup';
import LabeledInput from "../Elements/LabeledInput";
import CheckBox from "../Elements/CheckBox";
import Button from "../Elements/Button";
import AppSnackbar from "../Elements/AppSnackbar";
import { registerService } from "../../services/authService";

function FormSignUp() {
  const [isLoading, setIsLoading] = useState(false);
  const [snackbar, setSnackbar] = useState({
    open: false,
    message: "",
    severity: "success",
  });

  const handleCloseSnackbar = () => {
    setSnackbar({ ...snackbar, open: false });
  };

  const formik = useFormik({
    initialValues: {
      name: '',
      email: '',
      password: '',
      terms: false,
    },
    validationSchema: Yup.object({
      name: Yup.string().required('Name wajib diisi'),
      email: Yup.string()
        .email('Format email tidak valid')
        .required('Email wajib diisi'),
      password: Yup.string().required('Password wajib diisi'),
    }),
    onSubmit: async (values) => {
      setIsLoading(true);
      try {
        await registerService(values.name, values.email, values.password);
        setSnackbar({
          open: true,
          message: "Register Berhasil",
          severity: "success",
        });
        formik.resetForm();
      } catch (error) {
        setSnackbar({
          open: true,
          message: error.message || "Email sudah pernah digunakan sebelumnya", 
          severity: "error",
        });
      } finally {
        setIsLoading(false);
      }
    },
  });

  return (
    <>
      <h1 className="mt-5 text-3xl font-bold text-gray-800 dark:text-white text-center transition-colors duration-300">Create an account</h1>
      
      {/* form start */}
      <div className="mt-10">
        <form onSubmit={formik.handleSubmit}>
          <div className="mb-6">
            <LabeledInput
              label="Name"
              type="text"
              placeholder="Aji Seno"
              name="name"
              id="name"
              {...formik.getFieldProps('name')}
              error={formik.touched.name && formik.errors.name ? formik.errors.name : null}
            />
          </div>
          <div className="mb-6">
            <LabeledInput
              label="Email address"
              type="email"
              placeholder="hello@example.com"
              name="email"
              id="email"
              {...formik.getFieldProps('email')}
              error={formik.touched.email && formik.errors.email ? formik.errors.email : null}
            />
          </div>
          <div className="mb-6">
            <LabeledInput
              label="Password"
              type="password"
              placeholder="••••••••"
              name="password"
              id="password"
              {...formik.getFieldProps('password')}
              error={formik.touched.password && formik.errors.password ? formik.errors.password : null}
            />
          </div>
          <div className="mb-3">
            <CheckBox
              label="I agree to the Terms and Conditions"
              id="terms"
              type="checkbox"
              name="terms"
              {...formik.getFieldProps('terms')}
            />
          </div>
          <Button disabled={isLoading}>
            {isLoading ? "Loading..." : "Register"}
          </Button>
        </form>
      </div>
      {/* form end */}

      {/* teks start */}
      <div className="my-9 px-7 flex flex-col justify-center items-center text-xs text-gray-03 dark:text-gray-400 transition-colors duration-300">
        <div className="border border-gray-05 dark:border-[#404040] w-full transition-colors duration-300"></div>
        <div className="px-2 bg-special-mainBg dark:bg-[#181818] absolute transition-colors duration-300"> or sign up with</div>
      </div>
      {/* teks end */}
      
      {/* sign in with google start */}
      <div className="mb-8">
        <Button type="button" variant="secondary">
          <span className="flex items-center justify-center">
            <svg className="h-6 w-6 mr-2" xmlns="http://www.w3.org/2000/svg" width="800" height="800" viewBox="-0.5 0 48 48">
              <path d="M9.82727273,24 C9.82727273,22.4757333 10.0804318,21.0144 10.5322727,19.6437333 L2.62345455,13.6042667 C1.08206818,16.7338667 0.213636364,20.2602667 0.213636364,24 C0.213636364,27.7365333 1.081,31.2608 2.62025,34.3882667 L10.5247955,28.3370667 C10.0772273,26.9728 9.82727273,25.5168 9.82727273,24" fill="#FBBC05" />
              <path d="M23.7136364,10.1333333 C27.025,10.1333333 30.0159091,11.3066667 32.3659091,13.2266667 L39.2022727,6.4 C35.0363636,2.77333333 29.6954545,0.533333333 23.7136364,0.533333333 C14.4268636,0.533333333 6.44540909,5.84426667 2.62345455,13.6042667 L10.5322727,19.6437333 C12.3545909,14.112 17.5491591,10.1333333 23.7136364,10.1333333" fill="#EB4335" />
              <path d="M23.7136364,37.8666667 C17.5491591,37.8666667 12.3545909,33.888 10.5322727,28.3562667 L2.62345455,34.3946667 C6.44540909,42.1557333 14.4268636,47.4666667 23.7136364,47.4666667 C29.4455,47.4666667 34.9177955,45.4314667 39.0249545,41.6181333 L31.5177727,35.8144 C29.3995682,37.1488 26.7323182,37.8666667 23.7136364,37.8666667" fill="#34A853" />
              <path d="M46.1454545,24 C46.1454545,22.6133333 45.9318182,21.12 45.6113636,19.7333333 L23.7136364,19.7333333 L23.7136364,28.8 L36.3181818,28.8 C35.6879545,31.8912 33.9724545,34.2677333 31.5177727,35.8144 L39.0249545,41.6181333 C43.3393409,37.6138667 46.1454545,31.6490667 46.1454545,24" fill="#4285F4" />
            </svg>
            Continue with Google
          </span>
        </Button>
      </div>
      {/* sign in with google end */}
      
      {/* link start */}
      <div className="flex justify-center text-gray-03 dark:text-gray-400 transition-colors duration-300">
        <span className="text-sm mr-1">Already have an account?</span>
        <a href="/login" className="text-primary text-sm font-bold">
          Sign In Here
        </a>
      </div>
      {/* link end */}  

      <AppSnackbar
        open={snackbar.open}
        message={snackbar.message}
        severity={snackbar.severity}
        onClose={handleCloseSnackbar}
      />
    </>
  );
}

export default FormSignUp;